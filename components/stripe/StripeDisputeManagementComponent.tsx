...
const StripeDisputeManagementComponent = () => {
  ...
  useEffect(() => {
    const initializeStripeConnect = async () => {
      ...
      try {
        ...
        setError("Failed to load dispute management. Please try again later.");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="bg-white p-5 rounded-lg shadow-sm border border-gray-200">
      <h2 className="text-lg font-medium mb-3 text-gray-800">Dispute Management</h2>
      <div className="min-h-[400px]">
        {error ? (
          <div className="text-center text-red-500 py-4">{error}</div>
        ) : loading ? (
          <div className="text-center text-gray-600 py-4">Loading dispute management...</div>
        ) : !ryftSubAccountID ? (
...
export default StripeDisputeManagementComponent; 